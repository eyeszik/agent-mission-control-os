...
export function ArtifactPreviewPanel() {
  ...
  // Optimization: Derive selectedArtifact directly within the selector to avoid
  // re-rendering whenever other artifacts are added to the list.
  const selectedArtifact = useArtifactStore((state) => {
    if (!activeRunId) return null;
    const list = state.artifacts[activeRunId];
    if (!list) return null;
    return list.find(a => a.id === state.selectedArtifactId) || null;
  });
