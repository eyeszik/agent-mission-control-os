...
// Optimization: Hoist static layout calculations outside the component.
// These constants avoid allocating a new array and performing math on every render.
const STATIC_POSITIONS = AGENCY_PIPELINE_STAGES.map((stage, index) => ({
  stage,
  x: (index % COLS) * CELL_W + CELL_W / 2,
  y: Math.floor(index / COLS) * CELL_H + CELL_H / 2,
}));
const STATIC_WIDTH = COLS * CELL_W;
const STATIC_HEIGHT = Math.ceil(AGENCY_PIPELINE_STAGES.length / COLS) * CELL_H;
  ...
  return (
    <div className="flex-1 flex items-center justify-center p-8 relative">
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
        <svg className="w-full h-full" viewBox={`0 0 ${STATIC_WIDTH} ${STATIC_HEIGHT}`} preserveAspectRatio="xMidYMid meet" aria-label="Agency workflow">
          {STATIC_POSITIONS.slice(0, -1).map((position, index) => {
            const next = STATIC_POSITIONS[index + 1];
