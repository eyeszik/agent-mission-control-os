...
// This prevents the entire panel and other node items from re-rendering when a single node changes.
function LiveStepNode({ runId, node }: { runId: string, node: string }) {
    ...
    <div className="h-full bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4 flex flex-col">
      ...
      <div className="flex-1 flex flex-col gap-2 overflow-y-auto">
        ...
        {activeRunId && AGENCY_PIPELINE_STAGES.map((node) => (
          <LiveStepNode key={node} runId={activeRunId} node={node} />
        ))}
      </div>
    </div>
  );
