...
// Optimization: Extract a child component to subscribe only to its specific node's status,
// avoiding re-renders of the entire list whenever any single node updates.
function StepCheckpointItem({ runId, node }: { runId: string; node: string }) {
    ...
    <div className="h-full bg-zinc-900/40 border border-zinc-800/60 rounded-xl p-4 flex flex-col">
      ...
      <div className="flex-1 flex flex-col gap-2 overflow-y-auto">
        ...
        {activeRunId && AGENCY_PIPELINE_STAGES.map((node) => (
          <StepCheckpointItem key={node} runId={activeRunId} node={node} />
        ))}
      </div>
    </div>
  );
